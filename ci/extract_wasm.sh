...
if [ "${DRY_RUN}" != "true" ]; then
  ...
  echo "${image} -> mainnet_runtime WASM" > ./vflow_mainnet_runtime.compact.compressed.wasm
  echo "${image} -> volta_runtime WASM" > ./vflow_volta_runtime.compact.compressed.wasm
fi
